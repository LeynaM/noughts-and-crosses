from domain.value_objects.enums import Player
...
class Board:
    ...
    def make_move(self, position: Position, player: Player) -> None:
        if self._grid[position.row][position.col] is not None:
            raise PositionOccupiedError
        self._grid[position.row][position.col] = player

    def get_cell(self, position: Position) -> Player | None:
    ...
    def from_grid(self, grid: list[list[str | None]]) -> None:
        self._grid = [[Player(cell) if cell else None for cell in row] for row in grid]
